...
object ZChannel {
  ...
  class FiberList {
    ...
    def view(currentFiberId : FiberId) =
      deque
        .asScala
        .view
        .filter{fib =>
          (fib ne null) &&
          (fib.id != currentFiberId)  &&
          fib.isAlive()
        }

    def close()(implicit trace : Trace) : UIO[Unit] = {
      ZIO.fiberIdWith { fibId =>
        zio.Fiber.interruptAllAs(fibId)(/*deque.asScala.filter(fib => (fib.id != fibId) && fib.isAlive())*/ view(fibId))
      }
    }
  }
